package com.example.study.openDartApi.serviceImpl.stock;
...
import com.example.study.openDartApi.repository.stock.CorpUpdateRepository;
import com.example.study.openDartApi.repository.stock.CorporationRepository;
        ...
        while((LocalDate.now().getYear() - targetYear) < 5) {
            ...
            for (int idx = 0; idx < reprtCodes.length; idx++) {
                ...
                while(!"".equals(corpKeysStr)) {
                    log.info("=====dartKeyCountService :"+dartKeyCountService.getKey()+"============");
...
//                || 'K' == response.getBody().getCorpCls())
    ...
    private List<Corporation> getCorpKeys() throws LimitExceededException, InterruptedException, IOException, JDOMException {
        ...
        List<Corporation> output = new ArrayList<>();
        logger.info("===dartKeyCountService  :"+dartKeyCountService.getKey());
